...
def generate_data(exe):
    ...
    for req in range(1, 10):
        for res in range(1, 10):
            ...
            input_dir = "./input/"
            name_files = "%snames1.txt, %snames2.txt, %snames3.txt, %snames4.txt, %snames5.txt" % (
                input_dir, input_dir, input_dir, input_dir, input_dir)
            parameters = "%s %s" % (req, res)
            call_arguments = str(
                """["%s", "%s", "%s", "results.txt", "serviced.txt", "./input/names1.txt", "./input/names2.txt", "./input/names3.txt", "./input/names4.txt", "./input/names5.txt"]""" % ("./"+str(exe), req, res))
            # print(call_arguments)
            # Time the program using timeit
            time = timeit(stmt="subprocess.call(%s)" % call_arguments,
                          setup="import subprocess", number=reps) * T_CONVERSION / reps
...
def plot(data):
    ...
    surf2 = ax2.plot_surface(
        req2d, res2d, Z, cmap=plt.cm.coolwarm, linewidth=0)
